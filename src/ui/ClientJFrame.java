...
public class ClientJFrame extends javax.swing.JFrame {
    ...
    public ArrayList<Business> bizUndetermined;
    ...
    private void addTableListener() {
        busiTable.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent evt) {
                int indexSelected = busiTable.getSelectedRow();
                String status = bstm.getValueAt(indexSelected, bstm.getColumnCount() - 1).toString();
                if (status.equals("正在申请") || status.equals("已批准") || status.equals("已警告")) {
                    if (evt.getClickCount() == 2) {
                        if (JOptionPane.OK_OPTION == JOptionPane.showConfirmDialog(rootPane, "确认取消连接", "提醒", JOptionPane.OK_CANCEL_OPTION)) {
                            String req = "request:" + constant.getId() + ":" + "1";
...
//        Random rdm = new Random();
        String idCalled = this.cbRemoteId.getSelectedItem().toString();
        if (idCalled.equals(constant.getId())) {
            Object[] options = {"确定"};
            JOptionPane.showOptionDialog(this, "不能与自己建立连接", "提醒", JOptionPane.PLAIN_MESSAGE, JOptionPane.WARNING_MESSAGE, null, options, options[0]);
        } else {
            ...
            String videoSelected = cbVideo.getSelectedIndex() + "";
            bz.setVideoBusi(videoSelected);
            int videoBd = 0;
            if (videoSelected.equals("0")) {
                videoBd = 0;
            } else if (videoSelected.equals("1")) {
                videoBd = constant.getBdVideoNormal();
            } else if (videoSelected.equals("2")) {
...
//        System.out.println(cbVideo.getSelectedItem().toString());
            request += bz.getStationIDCalling() + ":0" + ":" + bz.getStationIDCalled() + ":" + bz.getBandwidth() + ":" + bz.getVoIPNum()
                    + ":" + bz.getVideoBusi() + ":" + bz.getFaxNum() + ":" + bz.getInterBandwidth() + ":" + bz.getAntennaCaliber()
...
//        int serial = Integer.parseInt(strs[strs.length-1]);
        String status = null;
//        System.out.println("has run here ,s:" + s);
...
//                System.out.println(cf);
            ...
            case 5:
                ...
                bz.setVideoBusi(strs[5]);
                bz.setFaxNum(strs[6]);
                bz.setInterBandwidth(strs[7]);
                bz.setAntennaCaliber(strs[8]);
                bz.setAmplifier(strs[9]);
                bz.setFreshTime((new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date()));
                bz.setAppStatus(status);
                bstm.addRecord(bz);
                break;
            case 6:
                status = "已取消";
